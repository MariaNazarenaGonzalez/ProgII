...
Cola c_ej7_atenderclientes(Cola c1, Cola c2, Cola c3, int tiempoatencion){
    Cola Cresu=c_crear();
    int Cliente1=0; int Cliente2=0; int Cliente3=0;
    int Ncliente1=0; int Ncliente2=0; int Ncliente3=0;
    while(!c_es_vacia(c1)||!c_es_vacia(c2)||!c_es_vacia(c3)){
        if(!c_es_vacia(c1)){
            if(Cliente1==0){
                Cliente1=(c_recuperar(c1))->clave; 
                Ncliente1++;
                c_desencolar(c1);
            }
            Cliente1=Cliente1-tiempoatencion;
            if(Cliente1==0){
                char * cc1; cc1=("Cola 1");
                c_encolar(Cresu,te_crear_con_valor(Ncliente1,cc1));
            }
        }
        if(!c_es_vacia(c2)){
            if(Cliente2==0){
                Cliente2=(c_recuperar(c2))->clave; 
                Ncliente2++;
                c_desencolar(c2);
            }
            Cliente2=Cliente2-tiempoatencion;
            if(Cliente2==0){
                char * cc2; cc2=("Cola 2");
                c_encolar(Cresu,te_crear_con_valor(Ncliente2,cc2));
            }
        }
        if(!c_es_vacia(c3)){
            if(Cliente3==0){
                Cliente3=(c_recuperar(c3))->clave; 
                Ncliente3++;
                c_desencolar(c3);
            }
            Cliente3=Cliente3-tiempoatencion;
            if(Cliente3==0){
                char * cc3; cc3=("Cola 3");
                c_encolar(Cresu,te_crear_con_valor(Ncliente3,cc3));
            }
        }
    }
    while(Cliente1>0||Cliente2>0||Cliente3>0){
        if(Cliente1>0){
            Cliente1=Cliente1-tiempoatencion;
            if(Cliente1==0){
                char * cc1; cc1=("Cola 1");
