...
void a_mostrar_pre_orden_avl(ArbolAVL AVL);
int main(){
    ArbolBinario a=a_crear();
    a=a_cargar(a);
    a_mostrar_pre_orden(a);
    ArbolAVL AVL= a_ej9_construiravl(a);
    a_mostrar_pre_orden_avl(AVL);
    printf("la diferencia de altura entre AVB y AB es de: %i",a_ej9_diferenciaalturas(a,AVL));
    return 0;
}
...
void a_mostrar_pre_orden_avl(ArbolAVL AVL){
    printf("Mostrar en pre_orden\n");
    a_mostrar_pre_ordenR(avl_raiz(AVL));
    printf("\n");
}
